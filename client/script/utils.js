...
function createMessageBubble(text, sender) {
    // The div to contain the span for message texts
    const messageDiv = document.createElement("div");
    messageDiv.classList.add("p-2", "rounded", "mb-2", "w-auto", "d-inline-block");
    if (sender === "user") {
        // For user messages: use a gray background with black text.
        messageDiv.classList.add("bg-custom-gray", "text-dark");
    } else {
        // For bot messages: use a white background with a black border and black text.
        messageDiv.classList.add("bg-white", "border", "border-dark", "text-dark");
    }

    // The span for message texts
